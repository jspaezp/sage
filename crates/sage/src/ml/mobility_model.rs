...
impl MobilityModel {
    ...
    pub fn fit(db: &IndexedDatabase, training_set: &[Feature]) -> Option<Self> {
        ...
        let residuals = ims
            ...
            .map(|(act, pred)| RESIDUAL_COEF * (act - pred))
            .collect::<Vec<_>>();
        ...
        let predicted_im = fp_predicted_im
            ...
            .map(|(pred, res)| pred + (res / RESIDUAL_COEF))
            .collect::<Vec<_>>();
